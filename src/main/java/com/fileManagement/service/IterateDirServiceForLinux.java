...
@Service
@Transactional
public class IterateDirServiceForLinux {
	@Autowired
	private FileNodeInfoDao fileNodeInfoDao;

	private List<String> filepath = new ArrayList<String>();

	private LinkedList<Object> fileinode = new LinkedList<Object>();
	...
	public DirBean getFiles(String dirPath) throws Exception {
		...
		Path userpath = Paths.get(dirPath);
		List<FileBean> filelist = new LinkedList<FileBean>();
		DirectoryStream<Path> paths = Files.newDirectoryStream(userpath);

		if (Files.exists(userpath)) {

			if (Files.isDirectory(userpath, LinkOption.NOFOLLOW_LINKS)) {
				...
				if (paths != null)
					for (Path p : paths) {
						...
						if (Files.isDirectory(p)) {
							...
							long sizefromdb = 0;
							List<FileManagementinfo> all = fileNodeInfoDao.findByFilePath(p.toString());
							if (all.isEmpty()) {

							} else {
								for (FileManagementinfo TmpFileManagementinfo : all) {
									sizefromdb = TmpFileManagementinfo.getFilesize();// 重复记录使用一次
									log.debug("通过数据库得到文件夹的，查找的路径是：" + p.toString() + "	Size:" + sizefromdb);
									break;
								}
							}
							...
							if (all.isEmpty()) {// No folder find in DB,get the
								// size by file system
								log.debug("即将计算Size的文件夹路径：" + p.toString());
								fileBean.setFileSize(FormetFileSize(foldersize = getFileSize(p)).toString());
								FileManagementinfo fileManagementinfo = new FileManagementinfo();
								fileManagementinfo.setFilePath(p.toString());
							...
							fileBean.setFileSize(FormetFileSize(getFileSizes(p)));

							fileBean.setFileSizeCompare(getFileSizes(p));
						}
				...
				dirSize = FormetFileSize(getFileSizes(userpath));
			}
		...
		log.debug("列出文件夹" + userpath.toString() + "里面所有的文件所需要的时间" + (System.currentTimeMillis() - StartTime) + "");
		return dirBean;
	}
	...
	// 取得文件大小,参数是Path
	public long getFileSizes(Path f) throws Exception {

		long s = 0;
		// log.debug("获取单个文件的大小...");
		try {
			if (Files.exists(f) && f != null) {
				s = Files.size(f);
			} else {
				log.debug("文件不存在");
				s = 0;
			...
			System.err.format("%s: no such file or directory%n", f.toString());
		}
	...
	// 取得文件夹大小,参数是Path
	public long getFileSize(Path f) throws Exception {
		long size = 0;
		if (Files.isSymbolicLink(f))
			return 0;
		if (f.toString().contains("/proc"))
			return 0;
		DirectoryStream<Path> paths = Files.newDirectoryStream(f);
			...
			for (Path p : paths) {
				if (Files.isDirectory(p)) {
					if (Files.isSymbolicLink(p)) {
						log.debug("跳过的链接文件夹：" + p.toString());
						continue;
					}
					size = size + getFileSize(p);
				...
				// bfa.fileKey());
				if (!fileinode.contains(bfa.fileKey()))
					fileinode.add(bfa.fileKey());
				else
					continue;
				size = size + getFileSizes(p);

			}

		// log.debug("文件夹大小：" + size);
		// log.debug("文件夹路径：" + f.toString());
		if ((Files.isDirectory(f)) && (!(Files.isSymbolicLink(f)))) {
			FileManagementinfo fileManagementinfo = new FileManagementinfo();
			fileManagementinfo.setFilePath(f.toString());
			fileManagementinfo.setFileProperty("Directory");
			fileManagementinfo.setFilesize(size);
			SaveFileToDb saveFileToDbThread = new SaveFileToDb(fileNodeInfoDao, fileManagementinfo);
			saveFileToDbThread.start();
